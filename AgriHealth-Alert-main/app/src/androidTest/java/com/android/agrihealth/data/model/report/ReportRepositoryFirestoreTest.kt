...
class ReportRepositoryFirestoreTest : FirebaseEmulatorsTest() {
    ...
    fun canGetReportsByFarmer() = runTest{
        ...
        assertEquals(3, repository.getReportsByFarmer(Firebase.auth.currentUser?.uid ?: "no current user").size)
    }
    ...
    fun canGetReportsByVet() = runTest {
        ...
        reports = reports.map {it.copy(farmerId = report1.farmerId, vetId = report1.vetId,  createdAt = now)}
    ...
    fun canEditReport() = runTest{
        ...
        val reports = repository.getReportsByFarmer(user1.uid)
        assertEquals(1, reports.size)
        assertEquals(editedReport1, reports.first().copy(farmerId = editedReport1.farmerId, createdAt = now))
    }
    ...
    fun deleteReportDeletesTheRightReport() = runTest {
        ...
        reports = reports.map {it.copy(farmerId = if (it.id == "1") report2.farmerId else report3.farmerId , createdAt = now)}
