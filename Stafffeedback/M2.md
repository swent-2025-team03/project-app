...
|------------------------------------------|--------------------------------|------------|-------------------------------------------|
| **Implementation (APK, code quality)**   | 5.35 out of 6 | 40%        | [See Details](#implementation-apk-code-quality) |
...
The APK is functional. An issue we encountered was that when refering to a user (vet or farmer) the app displays a firebase key, which is really not user friendly.

Overall the code is of good quality. You still let issues slip here and there and make it onto main. For example Sonar issues/code replication.The CI is setup correctly but on pull requests it always run twice. It's a detail but can be improved.


For this part, you received 5.35 points out of a maximum of 6.
...
The map doesn't really drive a main feature of the app yet. It isn't really a complete right now because we can't input a location for a report and when viewing a report on the map it doesn't center it around the location. 

Also the feature to mark a report as spam doesn't work.
...
Figma and architecture diagram are clear and up to date. Good job!
...
The Sprint Backlog is clean and the Product Backlog is furnished and up to date. We see you relgularly update the Product Backlog, this is great.
...
The last two meetings were really structured thanks to the job of the Scrum Master and the Product Owner.

In some of the earlier meeting you were still a bit unstructured and you weren't fully automous for M2. But we see great improvements lately.
...
In general good, but in Sprint 5, you focused mostly on fixes and small things so there weren't any major new features. You didn't deliver as much value as the two previous sprints. Ideally you should be more consistent with the fixes and also with the features.
